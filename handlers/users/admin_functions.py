    ...
    if message.text.isdigit():
        ...
        if get_amount >= 1:
            ...
            await message.answer("<b>✅ Пользователю</b> "
                                 ...
                                 f"<b>было выдано</b> <code>{get_amount}грн</code>",
                                 reply_markup=check_user_out_func(message.from_user.id))
            await bot.send_message(user_id, f"<b>💳 Вам было выдано</b> <code>{get_amount}грн</code>")
            await message.answer(search_user_profile(user_id), reply_markup=search_profile_func(user_id))
            await state.finish()
        else:
            await message.answer("<b>❌ Минимальная сумма выдачи 1грн</b>\n"
...
async def input_set_balance(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        ...
        if get_amount >= 0:
            ...
            await message.answer("<b>✅ Пользователю</b> "
                                 f"<a href='tg://user?id={get_user[1]}'>{get_user[3]}</a> "
                                 f"<b>был изменён баланс на</b> <code>{get_amount}грн</code>",
                                 reply_markup=check_user_out_func(message.from_user.id))
            await message.answer(search_user_profile(user_id), reply_markup=search_profile_func(user_id))
            await state.finish()
        else:
            await message.answer("<b>❌ Минимальная сумма баланса 0грн</b>\n"
    ...
    if message.text.startswith("+"):
        ...
        if get_input is not None:
            ...
            await message.answer(f"<b>📃 Чек:</b> <code>+{get_input[6]}</code>\n"
                                 ...
                                 f"💵 Сумма: <code>{get_input[5]}грн</code>\n"
                                 f"{way_input}\n"
        ...
        if get_purchase is not None:
            ...
            await message.answer(f"<b>📃 Чек:</b> <code>#{get_purchase[4]}</code>\n"
                                 ...
                                 f"💸 Цена 1-го товара: <code>{get_purchase[7]}грн</code>\n"
                                 f"💵 Сумма покупки: <code>{get_purchase[6]}грн</code>\n"
                                 f"👤 Купил товар: <a href='tg://user?id={get_purchase[1]}'>{get_purchase[3]}</a> <code>({get_purchase[1]})</code>\n"
                                 f"🔻 Баланс до покупки: <code>{get_purchase[11]}грн</code>\n"
                                 f"🔺 Баланс после покупки: <code>{get_purchase[12]}грн</code>\n"
