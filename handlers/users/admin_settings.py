...
async def get_id_for_change_contact(message: types.Message, state: FSMContext):
    ...
    if msg.isdigit():
        ...
        if get_status_user is None:
            ...
            await state.finish()
            msg = f"♻️ <b>По заменам товара писать: @l_criminologist_l <a href='tg://user?id={msg}'>@l_criminologist_l</a></b>\n\n" \
                  f"⚠️<b>И помни для получения замены товара присылайте после того как забрали товар сразу видео/фото подтверждающий что с товаром что то не так, а так же сообщение о покупке и товаре из бота😉</b>"
